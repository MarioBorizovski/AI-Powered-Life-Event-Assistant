...
export default function AdminUsersPage() {
  ...
  const handleRoleChange = async (userId: string, newRole: 'user' | 'admin') => {
    ...
    toast.success(`Улогата е променета на "${newRole === 'admin' ? 'Администратор' : 'Корисник'}"`)
  }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-20 lg:pb-0">
      <div>
        <h1 className="text-2xl md:text-3xl font-bold text-foreground">Управување со корисници</h1>
        ...
        <CardHeader>
          <CardTitle className="text-lg text-card-foreground flex items-center gap-2">
            ...
            {filteredUsers.length !== users.length && (
              <Badge variant="secondary">{filteredUsers.length} од {users.length}</Badge>
            )}
          </CardTitle>
          <CardDescription>
            ...
            <Table>
              ...
              <TableBody>
                {filteredUsers.map((userData) => (
                  <TableRow key={userData.id} className="cursor-pointer hover:bg-muted/50">
                    <TableCell className="font-medium">
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground font-mono">{userData.id.slice(0, 12)}...</p>
                        </div>
                      </div>
                    </TableCell>
                    ...
                    <TableCell>
                      ...
                      >
                        {userData.role === 'admin' ? 'Администратор' : 'Корисник'}
                      </Badge>
                    </TableCell>
                    <TableCell>
                      <Badge
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => handleViewUser(userData.id)}>
                            <Eye className="size-4" />
                            <span>Преглед детали</span>
                          </DropdownMenuItem>
                          <DropdownMenuSeparator />
                          <DropdownMenuItem
                            onClick={() => handleRoleChange(userData.id, userData.role === 'admin' ? 'user' : 'admin')}
                            disabled={userData.id === user?.id}
                          >
                            <UserCog className="size-4" />
                            <span>
                              {userData.role === 'admin' ? 'Направи корисник' : 'Направи администратор'}
                            </span>
                          </DropdownMenuItem>
                          <DropdownMenuSeparator />
                          <DropdownMenuItem
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Оваа акција не може да се поништи. Корисникот и сите негови податоци ќе бидат трајно избришани.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      {/* View User Dialog */}
      <Dialog open={!!viewUser} onOpenChange={() => { setViewUser(null); setUserRequests([]); }}>
          ...
          {isLoadingDetails ? (
            ...
            <Tabs defaultValue="info" className="py-4">
              <TabsList className="grid w-full grid-cols-2">
                ...
                <TabsTrigger value="requests">Барања ({userRequests.length})</TabsTrigger>
              </TabsList>
              ...
              <TabsContent value="info" className="space-y-6 mt-4">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className="text-xl font-semibold text-foreground">{viewUser.name}</h3>
                    <p className="text-muted-foreground">{viewUser.email}</p>
                    <p className="text-xs text-muted-foreground font-mono mt-1">ID: {viewUser.id}</p>
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      >
                        {viewUser.role === 'admin' ? 'Администратор' : 'Корисник'}
                      </Badge>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 rounded-lg bg-muted/50 border border-border">
                    ...
                    <p className="text-2xl font-bold text-foreground">{viewUser.requestsCount || 0}</p>
                  </div>
                  <div className="p-4 rounded-lg bg-muted/50 border border-border">
                    <div className="flex items-center gap-2 text-muted-foreground mb-1">
                    ...
                    <p className="text-2xl font-bold text-green-600 dark:text-green-400">{viewUser.completedRequestsCount || 0}</p>
                  </div>
                </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid sm:grid-cols-2 gap-4 p-4 rounded-lg bg-muted/50 border border-border">
                      <div>
                        ...
                        <p className="font-mono text-foreground">{viewUser.personalInfo.embg}</p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">Број на лична карта</p>
                        <p className="font-mono text-foreground">{viewUser.personalInfo.idCardNumber}</p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">Име и презиме</p>
                        <p className="text-foreground">{viewUser.personalInfo.firstName} {viewUser.personalInfo.lastName}</p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">Датум на раѓање</p>
                        <p className="text-foreground flex items-center gap-1">
                          <Calendar className="size-3" />
                          {new Date(viewUser.personalInfo.dateOfBirth).toLocaleDateString('mk-MK')}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">Место на раѓање</p>
                        <p className="text-foreground">{viewUser.personalInfo.placeOfBirth}</p>
                      </div>
                      <div>
                      ...
                      <div className="sm:col-span-2">
                        ...
                        <p className="text-foreground flex items-center gap-1">
                          ...
                          {viewUser.personalInfo.address}, {viewUser.personalInfo.postalCode} {viewUser.personalInfo.city}
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <TabsContent value="requests" className="mt-4">
                {userRequests.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-muted-foreground">Корисникот нема барања</p>
                  </div>
                ) : (
                    ...
                    {userRequests.map((request) => {
                      ...
                      return (
                        <Link
                          ...
                          onClick={() => { setViewUser(null); setUserRequests([]); }}
                        >
